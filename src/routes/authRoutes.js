...
const db = require('../config/database'); // JÁ É O POOL COM PROMESAS
...
    try {
        // CORRIGIDO: Agora usa db.query() diretamente
        const [rows] = await db.query('SELECT id_usuario, nome, email, senha, tipo FROM Usuarios WHERE email = ?', [email]); 
        if (rows.length === 0) {
...
        res.json({ auth: true, message: 'Login bem-sucedido.' }); 
...
        // CORRIGIDO: Agora usa db.query() diretamente
        await db.query('INSERT INTO Usuarios (nome, email, senha, tipo, criado_em) VALUES (?, ?, ?, ?, NOW())', [nome, email, hashedPassword, 'user']);
...
    try {
        // CORRIGIDO (Linha 84 original do erro): Agora usa db.query() diretamente
        const [rows] = await db.query('SELECT id_usuario, nome, email, tipo, criado_em, xp_total, fase_atual FROM Usuarios WHERE id_usuario = ?', [req.user.id]);
        if (rows.length === 0) {
            console.log('Usuário não encontrado');
...
        // CORRIGIDO: Agora usa db.query() diretamente
        await db.query('UPDATE Usuarios SET xp_total = ? WHERE id_usuario = ?', [totalXp, req.user.id]);
...
    try {
        // CORRIGIDO: Agora usa db.query() diretamente
        const [niveis] = await db.query('SELECT id, titulo, descricao, xp_total, posicao FROM niveis WHERE ativo = 1');
        if (!niveis.length) {
...
        if (email) {
            // CORRIGIDO: Agora usa db.query() diretamente
            const [existingUsers] = await db.query('SELECT id_usuario FROM Usuarios WHERE email = ? AND id_usuario != ?', [email, req.user.id]);
            if (existingUsers.length > 0) {
...
        // CORRIGIDO: Agora usa db.query() diretamente
        await db.query(`UPDATE Usuarios SET ${setClause} WHERE id_usuario = ?`, params);
...
        // CORRIGIDO: Agora usa db.query() diretamente
        const [rows_upper_plural] = await db.query('SELECT * FROM Usuarios LIMIT 1');
...
            // CORRIGIDO: Agora usa db.query() diretamente
            const [rows_lower_plural] = await db.query('SELECT * FROM usuarios LIMIT 1');
...
                // CORRIGIDO: Agora usa db.query() diretamente
                const [rows_lower_singular] = await db.query('SELECT * FROM usuario LIMIT 1');
